...
module SaveData
  ...
  def save_rentals(rentals)
    ...
    check_file('rentals')
    File.write('./storage/rentals.json', JSON.pretty_generate(rental_arr))
  end
end
...
module LoadData
  def load_books
    ...
    if File.exist?('./storage/books.json')
      data = File.read('./storage/books.json')
      if data != ''
  ...
  def load_people
    ...
    if File.exist?('./storage/people.json')
      data = File.read('./storage/people.json')
      if data != ''
  ...
  def load_rentals
    rentals = []
    return rentals unless File.exist?('./storage/rentals.json')

    file = File.open('./storage/rentals.json')
